...
\begin{definition}
    ...
    $X$ is limit point compact if every infinite set $A \subseteq X$ has a limit point in $X$. 
\end{definition}
...
\par Let $a\in A$ such that $U_a = X\setminus (A\setminus \{a\})$ is open and contains $a$. Then $\{U_a: a \in A\} \cup \{X\setminus A\}$ is an open covering of $X$. Since $X$ is compact, there exists a finite subcovering, say $\{U_{a_1}, U_{a_2}, \cdots, U_{a_n}\} \cup \{X\setminus A\}$. Thus,
\[ X = \bigcup_{i=1}^n U_{a_i} \cup (X\setminus A) = X\setminus (A\setminus \{a_1, a_2, \cdots, a_n\}), \]
which implies that $A$ is finite, a contradiction. Hence, $A$ has a limit point in $X$. \hfill $\blacksquare$
...
\begin{example}
    ...
    Let $X = \mathbb{N} \times \{p\}$ has no convergent subsequence. So $X$ is not sequentially compact. 
\end{example}

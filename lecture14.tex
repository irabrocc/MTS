...
\par \noindent \textbf{Step 3} Show that $c\in C$. Let $c\in A, A\in \mathcal{A}, \exists d\in A$ such that $d < c, [d, c] \subseteq A$. Since $[a, d]$ can be covered by finite elements(say $N$) from $\mathcal{A}$, $[a, c]$ can be covered by $N + 1$ elements from $\mathcal{A}$. So $c\in C$.

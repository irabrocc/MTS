...
\begin{example}
    Let $X = \{(x, y)| y = \sin(x)\} \cup  \{(0, y)| y \in [-1, 1]\} \subset \mathbb{R}^2$ with the Euclidean metric. Then $X$ is connected but not path-connected.
\end{example}
\par \noindent \textbf{Proof} 
\par First we prove that $X$ is path-connected. Let $p_1, p_2 \in X$ such that $P_1 = (0, 0)$ and $P_2 = (1, \sin(1))$. Let $\gamma: [0,1] \rightarrow X$ with $\gamma(t) = (x(t), y(t))$ such that $\gamma(0) = P_1$ and $\gamma(1) = P_2$. Since $\gamma$ is continuous, both $x(t)$ and $y(t)$ are continuous. 

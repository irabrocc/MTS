...
\chapter{Lecture 10: Frechet space. Hausdorff space. Continuity. }
...
\begin{property}
    $X$ is T1 if and only if $\forall x, y\in X, \exists U\ni x$ a open set such that $y \notin U$. That is, for all $x, y\in X$, there exists an open set containing $x$ but not containing $y$.
\end{property}
...
\par \noindent \textbf{Solution} No. Consider $X = \{a, b\}$ with the topology $\mathcal{T} = \{\emptyset, X, \{a\}\}$. 
